...
const cards = [
  ...
  },
  {
    ...
    description: 'עורך דין אייל פלשן, חבר בלשכת עורכי הדין בישראל החל משנת 2003. בוגר תואר ראשון במשפטים בהצטיינות מאוניברסיטת תל אביב ותואר שני מגיסטר במקרקעין מהטכניון. בעל ניסיון של למעלה מ-20 שנה בתחום המקרקעין, ביטוח ונזיקין, ליטיגציה אזרחית ודיני עבודה.',
...
function escapeHtml(text) {
  ...
  return String(text)
    .replace(/&/g, '&amp;')
    .replace(/"/g, '&quot;')  // Regular double quote (U+0022)
    .replace(/\u05F4/g, '&quot;')  // Hebrew geresh (U+05F4) - used in עו"ד
    .replace(/'/g, '&#39;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
}
